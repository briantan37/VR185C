...
static ovrCubeVertices cubeVertices2 =
        {
                // positions
                {
                        { -127, +127, -127, +127 }, { +127, +127, -127, +127 }, { +127, +127, +127, +127 }, { -127, +127, +127, +127 },	// top
...
static void ovrGeometry_CreateCube(ovrGeometry *geometry)
{
	...
	geometry->VertexAttribs[0].Index = VERTEX_ATTRIBUTE_LOCATION_POSITION;
 	...
 	geometry->VertexAttribs[0].Pointer = (GLvoid *)offsetof( ovrCubeVertices, positions );
	...
	geometry->VertexAttribs[1].Index = VERTEX_ATTRIBUTE_LOCATION_COLOR;
 	...
 	geometry->VertexAttribs[1].Pointer = (GLvoid *)offsetof( ovrCubeVertices, colors );
...
static void ovrScene_Create(ovrScene *scene, bool useMultiview)
{
	...
	{
	    //set to 0 so that there is no rotation
        scene->Rotations[i].x = 0.0;
        scene->Rotations[i].y = 0.0;
        scene->Rotations[i].z = 0.0;
	}
	...
	for ( int i = 0; i < NUM_INSTANCES; i++ )
	{
		...
		for ( ; ; )
		{
		    //set the initial position of the cube to (0,-3.1,-4.1)
		    rx = 0.0;
		    ry = -3.1;
		    rz = -4.1;
...
static void changeColorPointer(ovrScene * scene);

//This is called to update frame
											...
											const ovrTracking2 * tracking, ovrMobile * ovr )
{
    ...
    //pose quaternion variable;
    ovrQuatf pose;
    //vrapi_EnumerateInputDevices will have its DeviceID field set to the device ID of the enumerated controller
        ...
        if ( capsHeader.Type == ovrControllerType_TrackedRemote )
        {
            ...
            if ( vrapi_GetInputDeviceCapabilities( ovr, &remoteCaps.Header ) >= 0 )
            {
                ...
                if ( vrapi_GetCurrentInputState( ovr, capsHeader.DeviceID, &remoteState.Header ) >= 0 )
                {
                    ...
                    if(remoteState.IndexTrigger > 0) {
                        //Change the color to blue when pressed
                        changeColorPointer(scene);
                    }
                }
                ovrTracking trackingState;
                //vrapi_GetInputTrackingState will return the most recently-sampled pose
                if ( vrapi_GetInputTrackingState( ovr, capsHeader.DeviceID, 0, &trackingState) >= 0 ) {
                    //capture the orientation vector of the controller
                    pose = trackingState.HeadPose.Pose.Orientation;
                    ALOGV("[POSE] x=%f y=%f z=%f w=%f\n",pose.x, pose.y, pose.z, pose.w);
                    //found pose of controller
                }
            }
        }
    }
	...
	for ( int i = 0; i < NUM_ROTATIONS; i++ )
	{
	    //This is where to place the orientation vector of the controller so that the cube rotates in the direction
	    //I have to negate the values so that the rotation of the cube follows the controller
		rotationMatrices[i] = ovrMatrix4f_CreateRotation(-pose.x, -pose.y, -pose.z);
	}
...
static void changeColorPointer(ovrScene * scene) {
    cubeVertices = cubeVertices2;
    ovrScene_Destroy(scene);
    ovrScene_Create(scene, true);
}
